...
module.exports = {
	exportClassified: function(classified, done) {
		// console.log(classified[0]);
		var n = 0;
		var writeImages = function(currentSetOfImages, i) {
			lwip.create(28 * 10 + 28, 28 * 10 + 28, function(err, image) {
				var batch = image.batch();
				if (err) {
					return console.log("Something went wrong with image export: ", err);
				}
				var index = 0;
				for (var x = 0; x < 10; x++) {
					for (var y = 0; y < 10; y++) {
						if (index >= currentSetOfImages.length) {} else {
				...
				batch.writeFile(title, function(err) {
					if (err) {
						console.log("Something went wrong with the image export in the final stages: ", err);
					}
					n++;
					if (n > 9) {
						done();
		...
		for (var i = 0; i < 10; i++) {
			var currentSetOfImages = classified.filter(function(n) {
				return n[1] == i
			}).map(function(n) {
				return n[0];
			});
	...
	exportGrouped: function(centroids, allData, done, str) {
		...
		var belongs = allData.reduce(function(belongs, vector, index) {
			var index = centroids.reduce(function(old, centroid, inner_index) {
				...
				return (old.distance < dist) ? old : {
					index: inner_index,
					distance: dist
				};
			}, {
				index: 0,
				distance: distance(centroids[0], vector)
			}).index;
			belongs[index].push(vector);
			return belongs;
		}, centroids.map(function() {
			return []
		}))
