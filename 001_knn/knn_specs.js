	...
	describe('The train function works', function() {
		...
		*/
		it('concatenates the points handed to the train function onto the points already within it', function() {
			var knn = new KNN(10);
			//Making fake data
			var typeA = randomPoints(100, [1, 1], [0, 0]).map(function(n) {
				return [n, 1]
			});
			var typeB = randomPoints(100, [1, 1], [1, 0]).map(function(n) {
				return [n, 0]
			});
			knn.train(typeA);
		...
		*/
		describe('Optional helper functions to be used in predict and predictSingle', function() {
			...
			it('has function _distances, which works as expected', function() {
				...
				var distances = knn._distances([0, 0], [
					[
						[1, 0], 1
					]
				].concat(typeA.concat(typeB)));
		...
		it('has a predictSingle function, which takes a single vector', function() {
			...
			var typeB = randomPoints(100, [1, 1], [0, 1]).map(function(n) {
				return [n, 0]
			});
			knn.train(typeA);
			knn.train(typeB);
		...
		it('has a score function, which takes another set of the testing data and returns a number from 1 to 0', function() {
			...
			knn.train(typeA);
			knn.train(typeB);
			var typeB = randomPoints(100, [1, 1], [.95, 0]).map(function(n) {
				return [n, 1]
			});
...
describe('Testing the KNN with data from the MNIST', function() {
	...
	it('Can handle somewhat chaotic data', function() {
		var knn = new KNN(1);
		var typeA = randomPoints(1000, [1, 1], [0, 0]).map(function(n) {
			return [n, 0]
		});
		var typeB = randomPoints(1000, [1, 1], [.75, 0]).map(function(n) {
			return [n, 1]
		});
		knn.train(typeA);
		knn.train(typeB);
		var typeB = randomPoints(100, [1, 1], [0, 0]).map(function(n) {
			return [n, 0]
		});
    ...
    */
	it('Can be trained off the mnist data', function(done) {
		...
		var knn = new KNN(5);
		var trainingSet = allElements.slice(0, 1000); //Make the training set 
		var testingSet = allElements.slice(1000, 1100); //Make the testing set
		knn.train(trainingSet)
		var score = knn.score(testingSet);
		console.log("The program got a score of " + score + ", which means it got " + (score * 100) + "% correct.");

		var toClassify = testingSet.map(function(n) {
			return n[0]
		});
		var toExport = knn.predict(toClassify).map(function(n, index) {
			return [toClassify[index], n];
		});
